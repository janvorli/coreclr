...
PBYTE WriteBarrierManager::CalculatePatchLocation(LPVOID base, LPVOID label, int offset)
{
    ...
    return ((LPBYTE)s_barrierCopy + ((BYTE*)JIT_WriteBarrier - (BYTE*)JIT_PatchedCodeStart) + ((LPBYTE)GetEEFuncEntryPoint(label) - (LPBYTE)GetEEFuncEntryPoint(base) + offset));
}
...
int WriteBarrierManager::ChangeWriteBarrierTo(WriteBarrierType newWriteBarrier, bool isRuntimeSuspended)
{
    ...
    memcpy((BYTE*)s_barrierCopy + ((BYTE*)JIT_WriteBarrier - (BYTE*)JIT_PatchedCodeStart), (LPVOID)GetCurrentWriteBarrierCode(), GetCurrentWriteBarrierSize());
